...
## 后续修正：公司官网全页断行（2026-09-14）

- 联系区标题内容宽度从 460px 调整为 500px，使“从一个业务问题，开始一次有用的改变。”在桌面、中间宽度与手机端稳定保持两条完整语义行。
- 公司官网标题、重点短段落、方法步骤和咨询提示加入平衡换行；中文同时使用严格标点换行，并在支持的浏览器中启用短语级换行。701–820px 的开放内容区提前改为单列，移动端摘要获得更充足的排版宽度。
- 运行提交 `1e6cd4dd4cf69a11585dde166d7ca2b8b0a1e51b`；发布指纹 `2eb2b892a1d7`。国内已同步静态包；国际生产部署 `ffc76cd9-68f4-4a35-a286-27509a55088c`，来源提交和 `main` 一致。国际上一生产部署为 `a24781f3-0e96-4eb8-98cb-da38fcc30db4`。
- 实际浏览器覆盖 1391、1100、1025、960、920、900、880、850、830、821、820、768、700、600、390、370、360、320px；简体、繁体与英文首页，以及三语方案和案例页均无短尾行或横向溢出。国内与国际线上又分别在 1391px 和 320px 回读确认目标标题、线索猎手标题与页面宽度。
- 64 项行为测试、33 项动效测试通过；共享导航、SEO、Umami 配置、生产资源和全量上线检查通过。两地三语言主页面均返回 200，canonical、原 Umami 网站 ID、单次统计加载和新 CSS 内容与候选包一致。本次只更改排版样式，未制造线上统计事件。
- 发布前国内备份：`lanxin-official-direct:/var/backups/lancloudtech/20260914-040910/`，包含 `site.tar.gz` 与 `nginx.conf`。
