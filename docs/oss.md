...
海外同一入口走 Cloudflare R2 自定义域名 `https://files.lancloudtech.com/ai-course/...`（橙云，桶 `lan-ai-course`）。课件约 39MB，不能放进 Pages。维护：`npm run dns:files`；同步仍用 `sync-ai-course-downloads`（只从课程仓 `main` 拉取，不跟功能分支）。R2 凭证在 `~/.config/lanxin/env/cloudflare/r2-website.env`。
