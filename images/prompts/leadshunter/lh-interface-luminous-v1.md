...
- ID：`lh-interface-luminous-v1`
- 生成方式：内置 `image_gen` 编辑；无 CLI / API 调用。
- 日期：2026-09-13
- 编辑目标：`images/generated/leadshunter-page/lh-discovery-luminous-v1.png`
- 用途：公司官网首页、解决方案与实践中的完整 LeadsHunter 界面示意。
- 输出：`images/generated/leadshunter-page/lh-interface-luminous-v1.png`
- 尺寸：1536 × 1024（3:2）
- WebP：`lh-interface-luminous-v1.webp`（1536 × 1024）、`lh-interface-luminous-v1-768.webp`（768 × 512）；只做格式转换与缩放。
...
Input image: the supplied image is the edit target. It currently depicts a warm-white and jade lead-discovery dashboard inside a laptop.
...
Composition: preserve every interface component from the source: full left navigation icon rail from top circle to bottom dots; complete row of four KPI cards; entire four-column intent distribution panel; wide four-series trend chart and all endpoints; the complete five-row bottom status table. Preserve their relative order, arrangement and abstract chart paths. Fit ALL of these into a 1536×1024 landscape canvas with only a small equal warm-white breathing margin around the complete interface. The UI should occupy roughly 94% of the image width and height. Do not crop or omit the navigation, top row, chart, or bottom rows. Keep all modules flat with crisp orthographic edges.
Color palette: retain warm-white #f8faf7 and pale sage #e8f0e9 UI surfaces, deep teal and muted green-grey placeholder strokes, jade #358f75 and soft sage #91b8a0 accents, very restrained muted champagne #b8a574 for one series. Small outer margin uses matching warm-white/sage #f0f5f1. No blue or purple.
